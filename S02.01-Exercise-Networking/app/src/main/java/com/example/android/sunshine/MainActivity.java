...
//        }
    ...
    private void loadWeatherData() {
        ...
        new FetchWeatherDataAsynchTask().execute(url);
    }

    // TODO (5) Create a class that extends AsyncTask to perform network requests
    // TODO (6) Override the doInBackground method to perform your network requests
    // TODO (7) Override the onPostExecute method to display the results of the network request
    public class FetchWeatherDataAsynchTask extends AsyncTask<URL,Void,String> {

        @Override
        protected String doInBackground(URL... params) {
            String weatherDataJson = null;
            try {
                weatherDataJson = NetworkUtils.getResponseFromHttpUrl(params[0]);
            } catch (IOException e) {
                Log.e(FetchWeatherDataAsynchTask.class.getSimpleName(),"Error fetching weather data from the network",e);
                e.printStackTrace();
            }
